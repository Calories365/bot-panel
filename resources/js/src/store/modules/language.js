...
const getters = {
    ...
    [getterTypes.russianLanguageEnabled]: state => state.russianLanguageEnabled
};
...
const actions = {
    async [actionTypes.toggleRussianLanguage]({ commit, dispatch }, { enabled }) {
        ...
        try {
            ...
            const errorMessage = error.response?.data?.message || 'Failed to toggle Russian language';
            dispatch('addError', errorMessage, { root: true });
            commit(mutationTypes.toggleRussianLanguageFailure, error.response?.data || error);
            throw error;
        }
    }
};
